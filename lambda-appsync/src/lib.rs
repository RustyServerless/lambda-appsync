...
//!   - a `Handlers` trait with methods for wiring up the Lambda runtime.
//!
...
//! - **[`make_handlers!`]** — generates the `Handlers` trait for the Lambda runtime.
//!   Requires the `Operation` type from [`make_operation!`] to be in scope.
//!
//! ### Lambda runtime wiring
//!
//! - **[`default_service_fn!`]** — implement the `Handlers` trait generated by [`make_appsync!`]
//!   or [`make_handlers!`] with all the default methods on a unit struct and return the resulting
//!   `Handlers::service_fn` method that can be used by [`lambda_runtime::service_fn`].
//!
//! ### Resolver attribute
//!
...
//!
//! // Generate types, Operation enum, and Handlers trait from the schema
...
/// Implement the `Handlers` trait generated by [make_appsync] or
/// [make_handlers] with all the default methods on a unit struct
/// and return the resulting `service_fn` for use with [`lambda_runtime::service_fn`].
///
/// # Examples
/// ```rust,no_run
/// # use lambda_appsync::{tokio, lambda_runtime};
...
/// ```
#[macro_export]
macro_rules! default_service_fn {
    () => {{
        struct DefaultHandlers;
        impl Handlers for DefaultHandlers {}
        DefaultHandlers::service_fn
    }};
}
