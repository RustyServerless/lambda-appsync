...
## Internal `__operations` module

A `mod __operations` module is generated containing:

- Argument extraction functions for each operation
- Signature-checking functions used by [macro@appsync_operation] for compile-time validation
- The `DefaultOperations` trait that provides default implementations for all operations
...
- The [macro@appsync_operation] attribute macro depends on the `__operations` module generated by
  this macro. You must invoke `make_operation!` (or [make_appsync!]) before using
  `#[appsync_operation(...)]`.
