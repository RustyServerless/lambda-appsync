...
- **`event_hook` method on the `Handlers` trait**: a synchronous hook called before every operation dispatch that does nothing by default. Override it to implement pre-flight checks such as authentication. Returning `Some(response)` short-circuits the dispatch; returning `None` proceeds normally. This is aimed at easing the implementation of simple auth use cases by not requiring to study the internals of `Handlers::appsync_handler`.
- **`default_service_fn!()` macro**: creates an anonymous `Handlers` implementor with all default methods and returns its `service_fn` for use with `lambda_runtime::service_fn`. Replaces the previously generated `DefaultHandlers` unit struct.

### Changed

- **Breaking**: `make_handlers!` (and therefore `make_appsync!`) no longer generates a `DefaultHandlers` struct. Use `default_service_fn!()` instead:
  ```rust
  // Before
...
[0.11.0]: https://github.com/RustyServerless/lambda-appsync/compare/v0.10.0...v0.11.0
